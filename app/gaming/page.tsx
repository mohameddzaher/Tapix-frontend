...
export default function GamingPage() {
  ...
  useEffect(() => {
    ...
    timerRef.current = setTimeout(() => {
      ...
      router.push(`/gaming${search ? `?${search}` : ''}`, { scroll: false });
    }, 300);
    return () => {
      if (timerRef.current) clearTimeout(timerRef.current);
    };
  ...
  const FilterContent = () => (
    <>
      ...
      <FilterSection title={t('gaming.subcategories')} section="subcategories">
        <div className="space-y-2 max-h-48 overflow-y-auto pr-2">
          ...
          >
            <input
              ...
              className="sr-only"
            />
            <span className="text-sm">{t('gaming.allGaming')}</span>
          </label>
  ...
  const createCategoryMutation = useMutation({
    mutationFn: () =>
      adminApi.createCategory({
        ...
        descriptionAr: 'طوّر إعداد الألعاب الخاص بك',
        isActive: true,
        order: 0,
      }),
    onSuccess: () => {
      ...
      <div className="min-h-screen bg-beige-50 flex items-center justify-center px-4">
        ...
        >
          ...
          {isAdmin && (
            ...
            >
              {createCategoryMutation.isPending ? 'Creating...' : 'Create Gaming Category'}
            </Button>
          )}
        ...
        <div className="container-custom py-5">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div className="flex items-center gap-3">
              <h1 className="text-2xl font-display font-semibold text-dark-900">
                {filters.search ? `${t('products.resultsFor')} "${filters.search}"` : t('gaming.title')}
              </h1>
