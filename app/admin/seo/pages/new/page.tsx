...
import { HiOutlineArrowLeft, HiOutlineGlobe } from 'react-icons/hi';
import { Button, Input, Textarea, Card, Checkbox } from '@/components/ui';
import { adminApi } from '@/lib/api';
import { cn } from '@/lib/utils';
import toast from 'react-hot-toast';

export default function NewSEOPageConfigPage() {
  ...
  const createMutation = useMutation({
    mutationFn: (data: any) => adminApi.createSEOPage(data),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['admin-seo-pages'] });
      toast.success('Page SEO config created');
      router.push('/admin/seo/pages');
    },
    onError: (error: any) => {
      toast.error(error.response?.data?.message || 'Failed to create page config');
    },
  });
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-semibold text-dark-900">New Page SEO Config</h1>
          <p className="text-dark-500 mt-1">Configure SEO settings for a page</p>
        </div>
      </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          <Card padding="lg" className="space-y-6">
            ...
            <Input
              ...
              hint="The URL path this config applies to (e.g. /about)"
            />
